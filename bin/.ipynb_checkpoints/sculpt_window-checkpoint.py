...
def get_data(data_type, tracer, region, rp_cut, zrange, version="v0.4", kolim=(0.02, 0.2), korebin=10, ktmax=0.5, ktrebin=10, nran=5, cellsize=8, boxsize=8000, covtype='analytic'):
    ...
    if data_type ==  "secondGenMocksY1":
        ...
        c1 = np.loadtxt(os.path.join(covdir, 'cov_gaussian_prerec_ELG_LOPnotqso_GCcomb_{}_{}.txt'.format(zmin, zmax)))
        ...
        cov = c1_trunc #np.linalg.inv(np.linalg.inv(c1_trunc) + np.linalg.inv(c2_trunc))
        # d = "/global/cfs/cdirs/desi/survey/catalogs/Y1/LSS/iron/LSScats/v0.1/blinded/pk/covariances/"
...
def rotate_data(data_type, tracer, region, rp_cut, zrange, version='v0.4', ells=[0, 2, 4], kolim=(0.02, 0.2), korebin=10, ktmax=0.5, ktrebin=10, nran=5, cellsize=8, boxsize=8000, covtype="analytic", save=True):
    ...
    if save:
        ...
        window_fn = os.path.join(output_dir, 'wmatrix_smooth_{}_complete_gtlimaging_{}_{:.1f}_{:.1f}_default_lin{}{}_ells{}_{}cov_jaxfloat64.npy'.format(tracer, region, zmin, zmax, resinfo, '_rpcut{:.1f}_directedges'.format(rp_cut) if rp_cut else '', ''.join([str(i) for i in ells]), covtype))
        ...
        power_fn = os.path.join(output_dir, "pkpoles_{}_complete_gtlimaging_{}_{:.1f}_{:.1f}_default_lin{}{}_ells{}_{}cov_jaxfloat64.npy".format(tracer, region, zmin, zmax, resinfo, '_rpcut{:.1f}'.format(rp_cut) if rp_cut else '', ''.join([str(i) for i in ells]), covtype))
        np.save(power_fn, pknew)
        cov_fn = os.path.join(output_dir, "cov_{}_complete_{}_{:.1f}_{:.1f}{}{}_ells{}_{}cov_jaxfloat64.npy".format(tracer, region, zmin, zmax, resinfo, '_rp{:.1f}'.format(rpcut) if rpcut else '', ''.join([str(i) for i in ells]), covtype))
...
if __name__ == '__main__':
    ...
    rotate_data(data_type, tracer, region, rp_cut=rpcut, zrange=zrange, ells=ls, version=version, kolim=kolim, korebin=korebin, ktmax=0.5, ktrebin=ktrebin, covtype=covtype, save=True, nran=nran, cellsize=cellsize, boxsize=boxsize)
